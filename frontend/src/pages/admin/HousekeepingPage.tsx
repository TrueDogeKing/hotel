  ...
  const fetchDay = useCallback(
    (day: string) =>
      Promise.all([
        ...
        getHousekeepingRange(addDaysIso(day, -STRIP_BEFORE), addDaysIso(day, STRIP_AFTER)),
      ]),
    [],
  );

  const reload = useCallback(async () => {
    const [dayData, stripData] = await fetchDay(date);
    setDay(dayData);
    setStrip(stripData);
  }, [date, fetchDay]);
  ...
  return (
    <AdminLayout>
      ...
      <div className="hk-toolbar">
        <button type="button" onClick={() => setDate(addDaysIso(date, -1))}>
