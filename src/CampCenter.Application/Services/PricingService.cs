...
public class PricingService : IPricingService
{
    ...
    {
        Guard(request.PricePerPersonPerNightGrosze, request.DepositPerPersonPerNightGrosze);
    ...
    private static void Guard(long price, long deposit)
    {
        if (price is < 0 or > MaxRateGrosze || deposit is < 0 or > MaxRateGrosze)
        {
    ...
    private static PricingDefaultsDto ToDto(PricingDefaults d) =>
        new(d.PricePerPersonPerNightGrosze, d.DepositPerPersonPerNightGrosze, d.UpdatedAt);
}
