---
source_file: "frontend/src/pages/admin/UsersPage.tsx"
type: "code"
community: "useAuth"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/useAuth
---

# UsersPage.tsx

## Connections
- [[AdminLayout.tsx]] - `imports_from` [EXTRACTED]
- [[AdminUser]] - `imports` [EXTRACTED]
- [[App.tsx]] - `imports_from` [EXTRACTED]
- [[AuthContext.tsx]] - `imports_from` [EXTRACTED]
- [[ConfirmDialog()]] - `imports` [EXTRACTED]
- [[ConfirmDialog.tsx]] - `imports_from` [EXTRACTED]
- [[FormState_1]] - `contains` [EXTRACTED]
- [[UserRole]] - `imports` [EXTRACTED]
- [[UsersPage()]] - `contains` [EXTRACTED]
- [[admin.ts]] - `imports_from` [EXTRACTED]
- [[createUser()]] - `imports` [EXTRACTED]
- [[dates.ts]] - `imports_from` [EXTRACTED]
- [[deleteUser()]] - `imports` [EXTRACTED]
- [[emptyForm_3]] - `contains` [EXTRACTED]
- [[formatDate()]] - `imports` [EXTRACTED]
- [[getUsers()]] - `imports` [EXTRACTED]
- [[jwt.ts]] - `imports_from` [EXTRACTED]
- [[setUserPassword()]] - `imports` [EXTRACTED]
- [[setUserRole()]] - `imports` [EXTRACTED]
- [[useAuth()]] - `imports` [EXTRACTED]
- [[userRoles]] - `imports` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/useAuth