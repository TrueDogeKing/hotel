---
source_file: "frontend/src/api/public.ts"
type: "code"
community: "Public Booking Frontend (1)"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Public_Booking_Frontend_1
---

# public.ts

## Connections
- [[Availability]] - `contains` [EXTRACTED]
- [[AvailabilityCalendar]] - `contains` [EXTRACTED]
- [[AvailabilityDay]] - `contains` [EXTRACTED]
- [[BookingDetails]] - `contains` [EXTRACTED]
- [[BookingManagePage.tsx]] - `imports_from` [EXTRACTED]
- [[BookingPayment]] - `contains` [EXTRACTED]
- [[BookingSchedule.tsx]] - `imports_from` [EXTRACTED]
- [[BookingWizardPage.tsx]] - `imports_from` [EXTRACTED]
- [[CreateBookingInput]] - `contains` [EXTRACTED]
- [[CreateBookingResult]] - `contains` [EXTRACTED]
- [[PaymentReturnPage.tsx]] - `imports_from` [EXTRACTED]
- [[PublicClosure]] - `contains` [EXTRACTED]
- [[PublicPricing]] - `contains` [EXTRACTED]
- [[PublicSchedule]] - `contains` [EXTRACTED]
- [[PublicScheduleDay]] - `contains` [EXTRACTED]
- [[PublicScheduleEntry]] - `contains` [EXTRACTED]
- [[RangeCalendar.tsx]] - `imports_from` [EXTRACTED]
- [[api]] - `imports` [EXTRACTED]
- [[cancelBooking()]] - `contains` [EXTRACTED]
- [[client.ts]] - `imports_from` [EXTRACTED]
- [[createBooking()]] - `contains` [EXTRACTED]
- [[getAvailability()]] - `contains` [EXTRACTED]
- [[getAvailabilityCalendar()]] - `contains` [EXTRACTED]
- [[getBooking()]] - `contains` [EXTRACTED]
- [[getBookingSchedule()_1]] - `contains` [EXTRACTED]
- [[getPublicClosures()]] - `contains` [EXTRACTED]
- [[getPublicPricing()]] - `contains` [EXTRACTED]
- [[getStoredLanguage()]] - `imports` [EXTRACTED]
- [[index.ts]] - `imports_from` [EXTRACTED]
- [[validateMix()]] - `contains` [EXTRACTED]
- [[validateSplitMix()]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Public_Booking_Frontend_1