---
source_file: "frontend/src/utils/dates.ts"
type: "code"
community: "src / utils"
location: "L1"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/src_/_utils
---

# dates.ts

## Connections
- [[AdminBookingsPage.tsx]] - `imports_from` [EXTRACTED]
- [[BookingGroupSection.tsx]] - `imports_from` [EXTRACTED]
- [[BookingManagePage.tsx]] - `imports_from` [EXTRACTED]
- [[BookingSchedule.tsx]] - `imports_from` [EXTRACTED]
- [[BookingWizardPage.tsx]] - `imports_from` [EXTRACTED]
- [[CalendarTile.tsx]] - `imports_from` [EXTRACTED]
- [[ClosuresPage.tsx]] - `imports_from` [EXTRACTED]
- [[DateField.tsx]] - `imports_from` [EXTRACTED]
- [[DateRangeField.tsx]] - `imports_from` [EXTRACTED]
- [[DayCalendar.tsx]] - `imports_from` [EXTRACTED]
- [[DayTimetable.tsx]] - `imports_from` [EXTRACTED]
- [[GroupMealTimes.tsx]] - `imports_from` [EXTRACTED]
- [[GroupSchedulePanel.tsx]] - `imports_from` [EXTRACTED]
- [[HousekeepingPage.tsx]] - `imports_from` [EXTRACTED]
- [[MealTimesPage.tsx]] - `imports_from` [EXTRACTED]
- [[MonthCalendar.tsx]] - `imports_from` [EXTRACTED]
- [[OccupancyPage.tsx]] - `imports_from` [EXTRACTED]
- [[RangeCalendar.tsx]] - `imports_from` [EXTRACTED]
- [[ScheduleEntryForm.tsx]] - `imports_from` [EXTRACTED]
- [[SchedulePage.tsx]] - `imports_from` [EXTRACTED]
- [[UsersPage.tsx]] - `imports_from` [EXTRACTED]
- [[addDaysIso()]] - `contains` [EXTRACTED]
- [[dateFormatters]] - `contains` [EXTRACTED]
- [[dayNumber()]] - `contains` [EXTRACTED]
- [[formatDate()]] - `contains` [EXTRACTED]
- [[formatMonth()]] - `contains` [EXTRACTED]
- [[formatTimeRange()]] - `contains` [EXTRACTED]
- [[fromIsoDate()]] - `contains` [EXTRACTED]
- [[fromTimeInput()]] - `contains` [EXTRACTED]
- [[isoFromDayNumber()]] - `contains` [EXTRACTED]
- [[mondayIndex()]] - `contains` [EXTRACTED]
- [[monthFormatters]] - `contains` [EXTRACTED]
- [[monthGrid()]] - `contains` [EXTRACTED]
- [[shortWeekdays()]] - `contains` [EXTRACTED]
- [[toIsoDate()]] - `contains` [EXTRACTED]
- [[toTimeInput()]] - `contains` [EXTRACTED]
- [[todayIso()]] - `contains` [EXTRACTED]
- [[weekdayFormatters]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/src_/_utils