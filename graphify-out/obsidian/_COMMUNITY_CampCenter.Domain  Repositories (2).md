---
type: community
members: 11
---

# CampCenter.Domain / Repositories (2)

**Members:** 11 nodes

## Members
- [[BookingMealTime]] - code - frontend/src/api/admin.ts
- [[GroupMealTimes()]] - code - frontend/src/components/admin/GroupMealTimes.tsx
- [[GroupMealTimes.tsx]] - code - frontend/src/components/admin/GroupMealTimes.tsx
- [[NeighbourSitting]] - code - frontend/src/api/admin.ts
- [[Props_6]] - code - frontend/src/components/admin/GroupMealTimes.tsx
- [[clashingNeighbours()]] - code - frontend/src/components/admin/GroupMealTimes.tsx
- [[deleteBookingMeals()]] - code - frontend/src/api/admin.ts
- [[getBookingMealTimes()]] - code - frontend/src/api/admin.ts
- [[minutesOf()_1]] - code - frontend/src/components/admin/GroupMealTimes.tsx
- [[resetBookingMealTime()]] - code - frontend/src/api/admin.ts
- [[setBookingMealTime()]] - code - frontend/src/api/admin.ts

## Live Query (requires Dataview plugin)

```dataview
TABLE source_file, type FROM #community/CampCenterDomain_/_Repositories_2
SORT file.name ASC
```

## Connections to other communities
- 7 edges to [[_COMMUNITY_src  api (1)]]
- 3 edges to [[_COMMUNITY_Room Closure Management]]
- 2 edges to [[_COMMUNITY_useAuth]]
- 2 edges to [[_COMMUNITY_Frontend App Shell & i18n]]
- 1 edge to [[_COMMUNITY_Frontend Auth & API Client]]
- 1 edge to [[_COMMUNITY_src  utils]]
- 1 edge to [[_COMMUNITY_CampCenter.Application  Services (3)]]

## Top bridge nodes
- [[GroupMealTimes.tsx]] - degree 19, connects to 7 communities
- [[GroupMealTimes()]] - degree 4, connects to 2 communities
- [[getBookingMealTimes()]] - degree 3, connects to 1 community
- [[NeighbourSitting]] - degree 2, connects to 1 community
- [[BookingMealTime]] - degree 2, connects to 1 community